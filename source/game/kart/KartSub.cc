...
void KartSub::addFloor(const CollisionData &colData, bool) {
    ...
    auto &status = KartObjectProxy::status();
    if (colData.bMovingWaterV0 || colData.bMovingWaterV2) {
        ++m_movingWaterCollisionCount;

        if (colData.bMovingWaterV2) {
            status.setBit(eStatus::MovingWaterNotV2);
        } else {
            status.resetBit(eStatus::MovingWaterV2, eStatus::MovingWaterNotV2);
        }

        status.changeBit(colData.bMovingWaterDisableAccel, eStatus::DisableAcceleration);
        status.changeBit(colData.bMovingWaterVertical, eStatus::bMovingWaterVertical);
    } else {
        status.resetBit(eStatus::MovingWaterNotV2, eStatus::DisableAcceleration,
                eStatus::bMovingWaterVertical);
    }
...
void KartSub::calcMovingWater() {
    ...
    if (m_movingWaterCollisionCount > 0) {
        ...
        if (status.offBit(eStatus::MovingWaterNotV2)) {
        ...
        if (System::RaceConfig::Instance()->raceScenario().course == Course::Koopa_Cape &&
                status.onBit(eStatus::MovingWaterNotV2)) {
