...
ObjectWLWallGC::ObjectWLWallGC(const System::MapdataGeoObj &params) : ObjectKCL(params) {
    ...
    m_posOffset = m_initialPos - m_transform.base(2) * static_cast<f32>(params.setting(3));
...
const EGG::Matrix34f &ObjectWLWallGC::getUpdatedMatrix(u32 timeOffset) {
    ...
    time = time < m_startFrame ? 0 : (time - m_startFrame) % m_cycleDuration;
    ...
    m_currTransform.setBase(3, interpolate(t, m_initialPos, m_posOffset));
