...
void ObjectFireSnakeV::calcSub() {
    ...
    if (StateManager::m_currentStateId == 1 || m_currentStateId == 2 || m_currentStateId == 3 ||
            m_currentStateId == 4) {
        ++m_age;
...
void ObjectFireSnakeV::calcFalling() {
    ...
    if (StateManager::m_currentFrame > 5 && !getUnit()) {
        loadAABB(0.0f);
    }

    m_visualPos.x += m_bounceDir.x * m_fallSpeed;
    m_visualPos.z += m_bounceDir.z * m_fallSpeed;
    m_visualPos.y += INITIAL_Y_VELOCITY - GRAVITY * static_cast<f32>(StateManager::m_currentFrame);

    if (StateManager::m_currentFrame > 10) {
