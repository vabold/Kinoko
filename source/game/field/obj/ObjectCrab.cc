...
void ObjectCrab::init() {
    constexpr EGG::Vector3f INIT_ROT = EGG::Vector3f(0.0f, YAW, 0.0f);
...
void ObjectCrab::calcState() {
    ...
    if (m_statePhase == StatePhase::Start) {
        ...
        calcCurRot(EGG::Vector3f(0.0f, YAW, 0.0f));
        calcTransMat(m_curRot);
...
void ObjectCrab::calcTransMat(const EGG::Vector3f &rot) {
    ...
    EGG::Vector3f dir = m_railInterpolator->curTangentDir();
    EGG::Vector3f up = EGG::Vector3f::ey;
    EGG::Matrix34f basis;
    basis.makeOrthonormalBasisLocal(dir, up);
