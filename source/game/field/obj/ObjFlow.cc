...
ObjFlow::ObjFlow(const char *filename) {
    ...
    m_rawSlots = reinterpret_cast<u16 *>(
            reinterpret_cast<u8 *>(file) + sizeof(ObjAttrs) * m_count + sizeof(u16));
}

ObjFlow::~ObjFlow() = default;
...
u16 ObjFlow::getIdFromName(const char *name) const {
    for (s32 i = 0; i < m_count; i++) {
        if (strncmp(m_rawAttrs[i].name(), name, ObjAttrs::NAME_LENGTH) == 0) {
