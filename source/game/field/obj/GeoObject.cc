...
GeoObject::GeoObject(System::MapdataGeoObj *pMapDataGeoObj)
    : m_pos(pMapDataGeoObj->position()), m_scale(pMapDataGeoObj->scale()),
      m_rot(pMapDataGeoObj->rotation() * DEG2RAD), m_transformationMatrix(EGG::Matrix34f::ident) {
...
/// @addr{0x8081F9BC}
GeoObject::GeoObject(u16 id, const EGG::Vector3f &position, const EGG::Vector3f &rotation,
...
/// @addr{0x8081FB04}
GeoObject::GeoObject(const char *name, const EGG::Vector3f &position, const EGG::Vector3f &rotation,
        const EGG::Vector3f &scale)
    : m_pos(position), m_scale(scale), m_rot(rotation),
      m_transformationMatrix(EGG::Matrix34f::ident) {
...
GeoObjectDrivable::GeoObjectDrivable(System::MapdataGeoObj *pMapDataGeoObj)
    : GeoObject(pMapDataGeoObj) {
    m_type = 2;
}
...
GeoObjectKCL::GeoObjectKCL(System::MapdataGeoObj *pMapDataGeoObj)
    : GeoObjectDrivable(pMapDataGeoObj) {}
...
void GeoObjectKCL::initCollision() {
    ...
    m_bboxHalfSideLength = std::max(EGG::Mathf::abs(bboxHigh.x - bboxLow.x),
                                   EGG::Mathf::abs(bboxHigh.z - bboxLow.z)) *
            0.5f;
}
