...
ObjectCarTGE::ObjectCarTGE(const System::MapdataGeoObj &params)
    : ObjectCollidable(params), StateManager(this), m_auxCollision(nullptr), m_carName{},
      ...
      m_tangent(EGG::Vector3f::zero), m_curFloorNrm(EGG::Vector3f::zero),
      m_nextFloorNrm(EGG::Vector3f::zero) {
    m_carVariant = static_cast<u32>(params.setting(3));
    m_vel0 = static_cast<f32>(params.setting(2));
    m_vel1 = static_cast<f32>(params.setting(1));
...
void ObjectCarTGE::init() {
    ...
    if (speedSetting == 1) {
        m_railInterpolator->setCurrVel(m_vel0);
    } else if (speedSetting == 0) {
        m_railInterpolator->setCurrVel(m_vel1);
    }
...
f32 ObjectCarTGE::getCollisionRadius() const {
    ...
    return isTruck ? TRUCK_RADIUS : NORMAL_RADIUS;
}
...
/// @details On Moonview Highway, the speed cap m_vel0 is 70, which means this state is only
/// executed for 1 frame when cars get on the highway.
void ObjectCarTGE::calcState1() {
    m_currSpeed += HIGHWAY_SPEED_CHANGE;

    if (m_currSpeed > m_vel0) {
        m_currSpeed = m_vel0;
        m_nextStateId = 0;
    }
...
/// @details On Moonview Highway, the speed floor m_vel1 is 40, which means this state is only
/// executed for 1 frame when cars get off the highway.
void ObjectCarTGE::calcState2() {
    m_currSpeed -= HIGHWAY_SPEED_CHANGE;

    if (m_currSpeed < m_vel1) {
        m_currSpeed = m_vel1;
        m_nextStateId = 0;
    }
...
/// @addr{0x806D9430}
void ObjectCarTGE::calcFloorNrm() {
    m_curFloorNrm = m_railInterpolator->floorNrm(m_railInterpolator->curPointIdx());
    m_nextFloorNrm = m_railInterpolator->floorNrm(m_railInterpolator->nextPointIdx());
}
