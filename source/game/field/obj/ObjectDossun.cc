...
/// @addr{0x80764510}
ObjectDossun::ObjectDossun(const System::MapdataGeoObj &params) : ObjectCollidable(params) {}
...
void ObjectDossun::init() {
    ...
    m_fullDuration = m_fallDuration + GROUND_DURATION + m_riseDuration + BEFORE_FALL_DURATION;
...
Kart::Reaction ObjectDossun::onCollision(Kart::KartObject *kartObj, Kart::Reaction reactionOnKart,
    ...
    if (xzDist.length() < SQUISH_DISTANCE * m_scale.x &&
            (m_moveState == MoveState::Falling || m_touchingGround)) {
