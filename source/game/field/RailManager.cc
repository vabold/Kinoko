...
const Rail *RailManager::rail(size_t idx) const {
...
RailManager *RailManager::Instance() {
    return s_instance;
}
...
void RailManager::createPaths() {
    ...
    m_interpolatorTotal = m_pointCount + m_extraInterplatorCount;
    m_cameraCount = 1;

    // The base game indexes based on i and j, so we need to resize regardless of isObjectRoute.
    m_interpolators.resize(m_interpolatorTotal);
