...
void KPadGhostController::calcInner() {
    ...
    K_LOG("Buttons: %X\nSticks: %X\nTrick: %X", m_raceInputState.buttons, sticks,
            m_raceInputState.trickRaw);
}
...
KPadGhostButtonsStream::KPadGhostButtonsStream()
    : buffer(nullptr), sequenceCount(-1), size(0x276C), state(2) {}
...
KPadGhostFaceButtonsStream::KPadGhostFaceButtonsStream() : KPadGhostButtonsStream() {}

KPadGhostFaceButtonsStream::~KPadGhostFaceButtonsStream() = default;

KPadGhostDirectionButtonsStream::KPadGhostDirectionButtonsStream() : KPadGhostButtonsStream() {}

KPadGhostDirectionButtonsStream::~KPadGhostDirectionButtonsStream() = default;

KPadGhostTrickButtonsStream::KPadGhostTrickButtonsStream() : KPadGhostButtonsStream() {}
...
void KPadPlayer::setGhostController(KPadGhostController *controller, const u8 *inputs,
    ...
    if (inputs) {
        memcpy(m_ghostBuffer, inputs, 0x2774);
    }
