...
const MapdataCannonPoint::CannonParameter &MapdataCannonPoint::parameters() const {
    assert(m_paramterIdx <= 2);
    const s16 idx = m_paramterIdx < 1 ? 0 : m_paramterIdx;
    return CANNON_PARAMETERS[idx];
}
